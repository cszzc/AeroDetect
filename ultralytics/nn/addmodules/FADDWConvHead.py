...
class ADDWConvHead(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.cv3 = nn.ModuleList(
            nn.Sequential(
                nn.Sequential(AdaptiveDilatedDWConv(x, x, groups=math.gcd(x, x), kernel_size=3, stride=1, dilation=1),
                              Conv(x, c3, 1)),
                nn.Sequential(
                    AdaptiveDilatedDWConv(c3, c3, groups=math.gcd(c3, c3), kernel_size=3, stride=1, dilation=1),
                    Conv(c3, c3, 1)),
